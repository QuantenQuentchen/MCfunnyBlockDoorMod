...
object ModScreenHandlers {
    ...
    val FORCE_FIELD_SCREEN_HANDLER =
        Registry.register(
            Registries.SCREEN_HANDLER,
            Identifier(FunnyBlockDoorMod.MOD_ID, "force_field"),
            ExtendedScreenHandlerType(::ForceFieldScreenHandler)
        )
    private fun registerScreenHandler(name: String, screenHandler: ExtendedScreenHandlerType<*>) = Registry.register(
        Registries.SCREEN_HANDLER,
        Identifier(FunnyBlockDoorMod.MOD_ID, name),
        screenHandler
    )
